...
def env_for(prefix):
    ...
    e["PKG_CONFIG_PATH"] = ":".join(
        str(p) for p in (lib / "pkgconfig", lib64 / "pkgconfig", prefix / "share/pkgconfig")
    )
...
def main():
    ...
    for item in cfg["library"]:
        ...
        if not archive.exists():
            raise SystemExit(
                f"missing source archive: {archive}; run scripts/fetch_sources.py first"
            )
        ...
        if checksum != "unlocked":
            ...
            actual = h.hexdigest()
            if actual != expected:
                raise SystemExit(f"checksum mismatch: {archive}: {actual}")
        src = unpack(archive, sources / item["name"])
        src = src / item.get("source_subdir", "") if item.get("source_subdir") else src
        work = works / item["name"]
        ...
        if bs == "meson":
            ...
            run(["meson", "compile", "-C", work, f"-j{jobs}"], env=e)
            run(["meson", "install", "-C", work], env=e)
        elif bs == "autotools":
            if item.get("autogen"):
                run(item["autogen"].split(), cwd=src, env=e)
            work.mkdir(parents=True)
            run(
                [src / "configure", f"--prefix={prefix}", *item.get("configure", [])],
                cwd=work,
                env=e,
            )
            run(["make", f"-j{jobs}"], cwd=work, env=e)
            run(["make", "install"], cwd=work, env=e)
        elif bs == "cmake":
            run(
                [
                    "cmake",
            ...
            run(["cmake", "--build", work, "--parallel", str(jobs)], env=e)
            run(["cmake", "--install", work], env=e)
        else:
            raise SystemExit(f"unsupported build system: {bs}")
